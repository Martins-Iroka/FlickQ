...
import com.martdev.flickq.feature.admin.presentation.logic.rooms.RoomDetailAction
import com.martdev.flickq.feature.admin.presentation.logic.rooms.RoomDetailEvent
import com.martdev.flickq.feature.admin.presentation.logic.rooms.RoomDetailState
import com.martdev.flickq.feature.admin.presentation.logic.rooms.TheRoomDetail
...
fun RoomDetail() {
    RequireAdmin {
        AdminLayout(AdminNav.Rooms, "Room") {
            RoomDetailContent()
        }
    }
}
...
private fun RoomDetailContent() {
    ...
    val roomDataKey = RoomDataStorageKey("room_data")
    val roomData = sessionStorage.getItem(roomDataKey) ?: RoomData(name = "Nothing to show")
    val room = Room(roomData.id, roomData.name, roomData.rows, roomData.columns)
    val vm = rememberAdminViewModel<AdminRoomDetailViewModel> {
        parametersOf(room)
    }
    ...
    ObserveAsEvents(vm.event) { event ->
        when(event) {
            is RoomDetailEvent.NavigateToEditRoom -> {
                val room = event.roomData
                sessionStorage.setItem("name", room.name)
                sessionStorage.setItem("rows", room.rows.toString())
                sessionStorage.setItem("columns", room.columns.toString())
                ctx.router.navigateTo("/admin/room/info?id=${room.id}&mode=edit")
            }
        }
    }

    Box(modifier = Modifier.fillMaxWidth()) {
        RoomDetailView(state, state.detail, onAction)
...
private fun RoomDetailView(
    ...
    detail: TheRoomDetail,
    onAction: (RoomDetailAction) -> Unit
) {
    val room = detail.room ?: Room()
    Column(
        ...
        Row(
            ...
            Column(verticalArrangement = Arrangement.spacedBy(4.px)) {
                ...
                SpanText(
                    "Room ID: RM-${room.id} • Standard Configuration",
            ...
            Row(
                ...
                SecondaryButtonInline("Edit Room") { onAction(RoomDetailAction.OnEditClick(room)) }
                PrimaryButtonPlain("Generate Seats") {
                    onAction(
                        RoomDetailAction.OnGenerateSeatsClick(
                            room
                        )
                    )
                }
...
private fun StatCard(label: String, value: String) {
    Column(
        ...
        SpanText(
            label,
            Modifier.color(AdminColors.Muted).fontSize(11.px).fontWeight(FontWeight.SemiBold)
        )
        SpanText(
            value,
            Modifier.montserrat().color(AdminColors.BodyStrong).fontSize(28.px)
                .fontWeight(FontWeight.Bold)
        )
    }
}
...
@Composable
private fun StatusCard(detail: TheRoomDetail) {
    Column(
        modifier = Modifier.flexGrow(1).flexBasis(0.px)
            .backgroundColor(AdminColors.SurfaceAlt)
            .border(1.px, LineStyle.Solid, AdminColors.BorderWarm)
...
@Composable
private fun SeatMapCard(detail: TheRoomDetail, onAction: (RoomDetailAction) -> Unit) {
    Column(
        modifier = Modifier.fillMaxWidth()
            .backgroundColor(AdminColors.SurfaceAlt)
            .border(1.px, LineStyle.Solid, AdminColors.BorderWarm)
        ...
        when {
            ...
            seatsError != null -> ErrorBox(seatsError) { onAction(RoomDetailAction.OnRetrySeats) }
            detail.seats.isEmpty() -> Box(
                modifier = Modifier.fillMaxWidth().padding(32.px),
                contentAlignment = Alignment.Center
            ) {
                SpanText(
                    "No seats generated yet. Use “Generate Seats” to build the layout.",
                    Modifier.color(AdminColors.Muted).fontSize(14.px)
                )
            }
        ...
        byRow.forEach { (label, rowSeats) ->
            Row(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(6.px)) {
                    rowSeats.sortedBy { it.seatNumber }
                        .forEach { seat -> SeatCell(seat.seatNumber) }
                }
                RowLabelBox(label)
            }
        }
...
@Composable
private fun SeatInventoryCard(detail: TheRoomDetail) {
    ...
    Column(
        ...
        Row(
            ...
            SpanText(
                "Seat Inventory Reference",
                Modifier.montserrat().color(AdminColors.Heading).fontSize(18.px)
...
private fun SeatInventoryRow(seat: Seat) {
    Row(
        ...
        Cell(120.px) {
            Row(
                modifier = Modifier.backgroundColor(AdminColors.SuccessChip).borderRadius(9999.px)
                    .padding(topBottom = 3.px, leftRight = 10.px),
                verticalAlignment = Alignment.CenterVertically,
            ) {
                SpanText(
                    "Active",
                    Modifier.color(AdminColors.Success).fontSize(12.px)
                        .fontWeight(FontWeight.SemiBold)
                )
            }
        }
        ...
        Box(
            modifier = Modifier.fillMaxWidth().maxWidth(640.px).height(8.px)
                .styleModifier {
                    property(
                        "background",
                        "linear-gradient(90deg, rgba(229,9,20,0) 0%, rgba(229,9,20,0.6) 50%, rgba(229,9,20,0) 100%)"
                    )
...
@Composable
private fun GenerateSeatsConfirm(room: Room, onAction: (RoomDetailAction) -> Unit) {
    Overlay {
        SpanText(
            "Generate seats",
        ...
        Row(
            ...
            Box(modifier = Modifier.flexGrow(1)) {
                SecondaryButton("Cancel") {
                    onAction(
                        RoomDetailAction.OnDismissGenerateSeats
                    )
                }
            }
            Box(modifier = Modifier.flexGrow(1)) {
                PrimaryButtonPlain("Generate") {
                    onAction(
                        RoomDetailAction.OnConfirmGenerateSeats
                    )
                }
            }
        }
...
private fun seatCode(seat: Seat): String =
    "${seat.rowLabel}-${seat.seatNumber.toString().padStart(2, '0')}"